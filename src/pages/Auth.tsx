...
const Auth = () => {
  ...
  useEffect(() => {
    const checkUser = async () => {
      ...
      setEmail("");setPassword("");setName("");} catch (error: any) {toast({ title: "Sign Up Error", description: error.message || "An error occurred during registration", variant: "destructive" });} finally {setLoading(false);}};const handleSignIn = async (e: React.FormEvent) => {e.preventDefault();if (!email || !password) {toast({ title: "Error", description: "Please fill in all fields", variant: "destructive" });return;}setLoading(true);try {const response = await window.ezsite.apis.login({ email, password });if (response.error) {throw new Error(response.error);}toast({ title: "Success", description: "You have been successfully signed in" }); // Navigate to home page
      navigate("/");} catch (error: any) {toast({ title: "Sign In Error",
